...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Uint8Array format the Push API's subscribe() call actually needs.
  ...
  async function enablePushNotifications() {
    ...
    try {
      ...
      const keyData = await safeJson(keyResponse);
      if (!keyResponse.ok || !keyData.publicKey) {
        showToast("Push notifications aren't available right now.", "error");
        return false;
      }
      ...
      const response = await fetch(`${API_BASE_URL}/users/me/push-subscription`, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        showToast("Could not save your notification settings.", "error");
        return false;
      }

      localStorage.setItem("tynwald_push_enabled", "true");
      return true;
    } catch (err) {
  ...
  async function disablePushNotifications() {
    if (!("serviceWorker" in navigator)) {
      localStorage.setItem("tynwald_push_enabled", "false");
      return;
    }
    ...
    try {
      ...
      if (subscription) {
        ...
        await fetch(`${API_BASE_URL}/users/me/push-subscription`, {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${getToken()}`,
  ...
  const pushToggleLabel = document.getElementById("pushToggleLabel");

  function setPushToggleUI(enabled) {
    if (!pushToggle || !pushToggleLabel) return;
    pushToggle.classList.toggle("enabled", enabled);
    pushToggleLabel.textContent = enabled ? "Push notifications on" : "Enable push notifications";
  }

  if (pushToggle) {
    setPushToggleUI(localStorage.getItem("tynwald_push_enabled") === "true");

    pushToggle.addEventListener("click", async () => {
      const currentlyEnabled = localStorage.getItem("tynwald_push_enabled") === "true";
      pushToggle.disabled = true;

      if (currentlyEnabled) {
        await disablePushNotifications();
        setPushToggleUI(false);
        showToast("Push notifications turned off.", "info");
      } else {
        const success = await enablePushNotifications();
        setPushToggleUI(success);
        if (success) showToast("Push notifications enabled.", "success");
      }
